...
from dating.queries import get_user_interests
from dating.queries import get_max_id, get_interest_info
...
def extract_vals(user):
    ...
    interests.append(user.interest_id)
    interests.append(user.book_genre_id)
    interests.append(user.movie_genre_id)
    interests.append(user.music_genre_id)
    interests.append(user.fav_cuisine_id)
    interests.append(user.hobby_id)
    interests.append(user.religion_id)
    interests.append(user.outdoor_id)

    return interests


def calculate_coeffecient(difference, max_id, point_values):
    """ returns a value % for a specific quality"""
    return ((max_id - difference) / float(max_id)) * point_values
...
def make_match(user_id_1, user_id_2):
    ...
    if len(vals1) == len(vals2):
        ...
        user_1_fav_cuisine_id = vals1[4]
        user_2_fav_cuisine_id = vals2[4]
        ...
        user_1_hobby_id = vals1[5]
        user_2_hobby_id = vals2[5]
        ...
        user_1_religion_id = vals1[6]
        user_2_religion_id = vals2[6]
        ...
        user_1_outdoor_id = vals1[7]
        user_2_outdoor_id = vals2[7]
    ...
    if commons == []:
        ...
        for item in commons:
            ...
            if item:
                ...
                if item[1] == 1:
                    common_book_gen = common_interest.book_genre_name
                    if common_book_gen == "I will read anything":
                        d1 = "You both love to read."
                        common_items.append(d1)
                    if common_book_gen == "Don't read at all":
                        d1 = "You both dislike reading."
                        common_items.append(d1)
                    else:
                        d1 = "You both like to read " + common_book_gen
                        d2 = " books."
                        common_items.append(d1 + d2)
                if item[1] == 2:
                    common_movie_gen = common_interest.movie_genre_name
                    if common_movie_gen == "Will watch anything.":
                        d1 = "You both love to watch movies."
                        common_items.append(d1)
                    if common_movie_gen == "Not that into movies":
                        d1 = "You both dislike movies."
                        common_items.append(d1)
                    else:
                        d1 = "You both like to watch " + common_movie_gen
                        d2 = " movies."
                        common_items.append(d1 + d2)
                if item[1] == 3:
                    common_music_gen = common_interest.music_genre_name
                    if common_music_gen == "Will listen to anything":
                        d1 = "You both love Music."
                        common_items.append(d1)
                    if common_music_gen == "No Music":
                        d1 = "You both dislike music."
                        common_items.append(d1)
                    else:
                        d1 = "You both like to listen to " + common_music_gen
                        d2 = " music."
                        common_items.append(d1 + d2)
                if item[1] == 4:
                    common_cuisine_name = common_interest.fav_cuisine_name
                    if common_cuisine_name == "I am not very experimental":
                        common_items.append("You both are not very experimental with foods.")
                    if common_cuisine_name == "I love them all":
                        common_items.append("You both love all types of foods.")
                    else:
                        d1 = "You both enjoy " + common_cuisine_name + " food."
                        common_items.append(d1)
                if item[1] == 5:
                    com_hobby_name = common_interest.hobby_name
                    d1 = "You both share the same interest of " + com_hobby_name + "."
                    common_items.append(d1)
                if item[1] == 6:
                    common_religion = common_interest.religion_name
                    common_items.append("You are both " + common_religion + ".")
                if item[1] == 7:
                    common_outdoors = common_interest.outdoor_activity
                    if common_outdoors == "I am pretty adventurous":
                        common_items.append("You both love outdoors.")
                    if common_outdoors == "I hate outdoors":
                        common_items.append("You both hate outdoors.")
                    else:
                        common_items.append("You both enjoy" + common_outdoors + ".")
